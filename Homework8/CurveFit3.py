...
class CurveData(Dataset):
    def __init__(self):
        df = pd.read_csv("func3_output.csv")  # Load our dataset
...
class CurveFit(nn.Module):
    def __init__(self):
        ...
        self.in_to_h1 = nn.Linear(1, 32)
        self.h1_to_out = nn.Linear(32, 1)
...
with torch.no_grad():
    ...
    y_linear = cn(torch.linspace(1,2,steps=1000).view(-1,1)).view(-1)
...
#for our function
plt.plot(x,np.cos(x)**2*np.e**(x),c='black')
