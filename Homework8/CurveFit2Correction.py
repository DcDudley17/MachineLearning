...
class CurveFit(nn.Module):
    def __init__(self):
        ...
        self.in_to_h1 = nn.Linear(1, 128)
        self.h1_to_h2 = nn.Linear(128, 128)
        self.h2_to_out = nn.Linear(128, 1)

    def forward(self, x):
        x = torch.tanh(self.in_to_h1(x))
        x = torch.tanh(self.h1_to_h2(x))
        x = self.h2_to_out(x)
        return x
